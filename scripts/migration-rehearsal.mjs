 ...
 *
 *   - clean apply 0001 -> 0035 in order
...
function run() {
  ...
  ok("migration files present", files.length === 35, `found ${files.length} migration files, expected 35`);

  // 1) Clean apply 0001 -> 0035 on a fresh database.
  ...
  ok("clean apply 0001->0035 (57 public tables)", tableCount === 57, `got ${tableCount} public tables`);
  ...
  const checkpoints = [
    ...
    ["pre-reading-ingestion", 31], ["pre-reading-originals", 32], ["pre-reading-semantic", 33], ["current", 35],
  ];
