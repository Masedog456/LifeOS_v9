...
/**
 * Entity workspace self-tests (LIFEOS-029) — developer route.
 *
 * Runs `lib/entities/selftest.ts` and exposes a machine-readable summary at
 * `#entity-selftest-summary` for the `entity.mjs` E2E suite. Client-only
 * (timings), self-contained, never touches the user's store.
 */

import { useMemo, useSyncExternalStore } from "react";
import { runEntitySelfTests } from "@/lib/entities/selftest";

export default function EntityTestsPage() {
  const mounted = useSyncExternalStore(() => () => {}, () => true, () => false);
  const report = useMemo(() => (mounted ? runEntitySelfTests() : null), [mounted]);

  if (!report) {
    return (
      <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
        <h1 className="text-2xl font-semibold tracking-tight">Entity self-tests</h1>
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
      <header className="mb-5">
        <h1 className="text-2xl font-semibold tracking-tight">Entity self-tests</h1>
        <p className="mt-1 text-sm text-zinc-500">Relationships, backlinks, activity timeline, context, previews/hover cards, graph neighbors, navigation memory, and performance.</p>
      </header>

      <div
        id="entity-selftest-summary"
      ...
      >
        <p className="font-medium">{report.pass ? "✓ All entity self-tests pass" : "✗ Some entity self-tests failed"}</p>
