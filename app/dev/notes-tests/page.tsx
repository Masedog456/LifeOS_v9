"use client";
/** Notes, Topics & Capture front-door self-tests (LIFEOS-052). Dev-only surface. */
import { useMemo } from "react";
import { runNotesSelfTests } from "@/lib/notes/selftest";

export default function NotesTestsPage() {
  // Pure and synchronous — no effect, no state, no cascading render.
  const report = useMemo(() => runNotesSelfTests(), []);
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-10">
      <h1 className="text-2xl font-semibold tracking-tight">Notes self-tests</h1>
