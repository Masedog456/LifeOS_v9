...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 32", releaseVersions().migrationVersion === 32);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 32 }, (_, i) => i + 1));
  ok("2.1 dense 1..32 valid", dense.ok, dense.problems.join("; "));
  ok("2.2 duplicate number rejected", !validateMigrationList([1, 1, 2]).ok);
  ok("2.3 gap rejected", !validateMigrationList([1, 3]).ok);
  ok("2.4 wrong count rejected", !validateMigrationList(Array.from({ length: 31 }, (_, i) => i + 1)).ok);
  ok("2.5 eight checkpoints", MIGRATION_CHECKPOINTS.length === 8);
  ok("2.6 checkpoints cover required ids", ["pre-reading", "pre-workspaces", "pre-actions", "pre-planning", "pre-maintenance", "pre-security", "pre-reading-ingestion", "current"].every((c) => MIGRATION_CHECKPOINTS.some((m) => m.id === c)));
  ok("2.7 only 0033 release fix allowed", isAllowedReleaseFixMigration("0033_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0034_extra.sql"));
  ...
  ok("4.2 40 data domains", inv.dataDomainCount === 40);
