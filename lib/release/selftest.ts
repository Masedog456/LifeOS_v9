...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 31", releaseVersions().migrationVersion === 31);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 31 }, (_, i) => i + 1));
  ok("2.1 dense 1..31 valid", dense.ok, dense.problems.join("; "));
  ok("2.2 duplicate number rejected", !validateMigrationList([1, 1, 2]).ok);
  ok("2.3 gap rejected", !validateMigrationList([1, 3]).ok);
  ok("2.4 wrong count rejected", !validateMigrationList(Array.from({ length: 30 }, (_, i) => i + 1)).ok);
  ok("2.5 seven checkpoints", MIGRATION_CHECKPOINTS.length === 7);
  ok("2.6 checkpoints cover required ids", ["pre-reading", "pre-workspaces", "pre-actions", "pre-planning", "pre-maintenance", "pre-security", "current"].every((c) => MIGRATION_CHECKPOINTS.some((m) => m.id === c)));
  ok("2.7 only 0032 release fix allowed", isAllowedReleaseFixMigration("0032_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0033_extra.sql"));
