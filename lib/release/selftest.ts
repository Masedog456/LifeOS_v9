...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 41", releaseVersions().migrationVersion === 41);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 41 }, (_, i) => i + 1));
  ok("2.1 dense 1..41 valid", dense.ok, dense.problems.join("; "));
  ...
  ok("2.5 thirteen checkpoints", MIGRATION_CHECKPOINTS.length === 13); // +pre-constitution (056) +pre-successor-cascade (056D) +pre-calendar (067)
  ...
  ok("2.7 only the 0042 release fix is allowed", isAllowedReleaseFixMigration("0042_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0043_extra.sql"));
