...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 37", releaseVersions().migrationVersion === 37);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 37 }, (_, i) => i + 1));
  ok("2.1 dense 1..37 valid", dense.ok, dense.problems.join("; "));
  ...
  ok("2.7 only 0038 release fix allowed", isAllowedReleaseFixMigration("0038_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0039_extra.sql"));
  ...
  ok("4.2 42 data domains", inv.dataDomainCount === 42); // +notes (052) +protocols (054)
