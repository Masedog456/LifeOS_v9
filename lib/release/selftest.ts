...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 45", releaseVersions().migrationVersion === 45);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 45 }, (_, i) => i + 1));
  ok("2.1 dense 1..45 valid", dense.ok, dense.problems.join("; "));
  ...
  ok("2.5 seventeen checkpoints", MIGRATION_CHECKPOINTS.length === 17); // +pre-constitution (056) +pre-successor-cascade (056D) +pre-calendar (067) +pre-integrations (068) +pre-due-time-recurrence (074) +pre-cas-guard (076B)
  ...
  ok("2.7 only the 0046 release fix is allowed", isAllowedReleaseFixMigration("0046_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0047_extra.sql"));
  ok("2.8 …and the number the head now occupies is NOT a free hatch", !isAllowedReleaseFixMigration("0045_v1_release_fix.sql"));
  ...
  ok("4.2 46 data domains", inv.dataDomainCount === 46, `${inv.dataDomainCount}`); // +notes (052) +protocols (054) +constitution ×2 (056) +events & recurrenceCompletions (061)
  ...
  // Fed the constant, not a literal: this asserts the gates pass at the head
  // this build actually ships, and follows it when the head advances.
  const ev = gatherEvidence(EXPECTED_MIGRATION_VERSION);
