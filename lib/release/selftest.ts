...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 43", releaseVersions().migrationVersion === 43);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 43 }, (_, i) => i + 1));
  ok("2.1 dense 1..43 valid", dense.ok, dense.problems.join("; "));
  ...
  ok("2.5 fifteen checkpoints", MIGRATION_CHECKPOINTS.length === 15); // +pre-constitution (056) +pre-successor-cascade (056D) +pre-calendar (067) +pre-integrations (068) +pre-due-time-recurrence (074)
  ...
  ok("2.7 only the 0044 release fix is allowed", isAllowedReleaseFixMigration("0044_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0045_extra.sql"));
  ok("2.8 …and the number the head now occupies is NOT a free hatch", !isAllowedReleaseFixMigration("0043_v1_release_fix.sql"));
