...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 36", releaseVersions().migrationVersion === 36);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 36 }, (_, i) => i + 1));
  ok("2.1 dense 1..36 valid", dense.ok, dense.problems.join("; "));
  ok("2.2 duplicate number rejected", !validateMigrationList([1, 1, 2]).ok);
  ok("2.3 gap rejected", !validateMigrationList([1, 3]).ok);
  ok("2.4 wrong count rejected", !validateMigrationList(Array.from({ length: 35 }, (_, i) => i + 1)).ok);
  ...
  ok("2.7 only 0037 release fix allowed", isAllowedReleaseFixMigration("0037_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0038_extra.sql"));
  ...
  ok("4.2 41 data domains", inv.dataDomainCount === 41); // +notes (LIFEOS-052)
