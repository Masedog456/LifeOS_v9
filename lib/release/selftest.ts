...
export function runReleaseSelfTests(): SelfTestReport {
  ...
  ok("1.4 migration version 35", releaseVersions().migrationVersion === 35);
  ...
  // ---- 2. Migrations ----
  const dense = validateMigrationList(Array.from({ length: 35 }, (_, i) => i + 1));
  ok("2.1 dense 1..35 valid", dense.ok, dense.problems.join("; "));
  ok("2.2 duplicate number rejected", !validateMigrationList([1, 1, 2]).ok);
  ok("2.3 gap rejected", !validateMigrationList([1, 3]).ok);
  ok("2.4 wrong count rejected", !validateMigrationList(Array.from({ length: 34 }, (_, i) => i + 1)).ok);
  ...
  ok("2.7 only 0036 release fix allowed", isAllowedReleaseFixMigration("0036_v1_release_fix.sql") && !isAllowedReleaseFixMigration("0037_extra.sql"));
