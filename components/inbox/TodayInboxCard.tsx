  ...
  return (
    <section aria-label="Capture inbox" className="rounded-2xl border border-black/[.08] p-4 dark:border-white/[.10]">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Capture inbox</h2>
        <Link href="/process" className="text-[11px] text-zinc-500 dark:text-zinc-400 underline-offset-4 hover:underline">Open inbox →</Link>
      </div>
      <p className="mt-1.5 text-sm text-zinc-800 dark:text-zinc-100">
        {counts.inbox} capture{counts.inbox === 1 ? "" : "s"} to process
        {oldest > 0 && <span className="text-zinc-500 dark:text-zinc-400"> · oldest {oldest}d</span>}
      </p>
      {returning > 0 && <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">{returning} deferred item{returning === 1 ? "" : "s"} returned today.</p>}
      {next && (
