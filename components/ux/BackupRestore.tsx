...
import { validateBackup, previewRestore, applyRestore, type RestoreMode, type RestoreValidation } from "@/lib/ux/restore";
...
export default function BackupRestore() {
  ...
  const preview = validation?.ok && validation.backup ? previewRestore(state, validation.backup, mode) : null;
  ...
  const doImport = () => {
    ...
    requestConfirm({
      ...
      onConfirm: () => {
        ...
        toast({ kind: "success", message: mode === "overwrite" ? "Data restored (overwrite)" : "Data merged in", detail: `${total} records in backup` });
      },
      impact: {
        name: fileName || "backup file", typeLabel: "Restore",
        children: [], undoable: false, severity: "high", verb: mode === "overwrite" ? "Overwrite" : "Merge",
        linkedNote: mode === "overwrite"
          ? "Overwrite REPLACES all current records with the backup’s. Export your current data first if unsure."
          : "Merge adds the backup’s records, and updates any with a matching id. Your other records are kept.",
      },
    });
  };
        ...
        <div className="mt-4 rounded-lg border border-black/10 p-3 text-xs dark:border-white/12">
          ...
          {validation.errors.length > 0 ? (
            <div className="mt-1 text-rose-600 dark:text-rose-400" role="alert">
              ...
              <ul className="mt-0.5 list-inside list-disc">{validation.errors.map((e, i) => <li key={i}>{e}</li>)}</ul>
              <p className="mt-1 text-zinc-500">Your current data is unchanged.</p>
            </div>
          ) : (
            <>
              <p className="mt-1 text-zinc-500">Schema v{validation.version} · {totalRecords(validation.counts)} records in file.</p>
              {validation.warnings.map((w, i) => <p key={i} className="mt-0.5 text-amber-600 dark:text-amber-400">⚠ {w}</p>)}
              ...
              {preview && (
                <div className="mt-2 max-h-40 overflow-auto rounded border border-black/[.06] dark:border-white/[.08]">
                  <table className="w-full text-left text-[11px]">
                    ...
                    <tbody>
                      {preview.domains.filter((d) => d.current || d.incoming).map((d) => (
