...
export default function ExplanationDetail({ explanation, defaultOpen = false }: { explanation: MemoryExplanation; defaultOpen?: boolean }) {
  return (
    <details className="group mt-2" open={defaultOpen}>
      <summary className="cursor-pointer list-none text-[11px] text-zinc-500 dark:text-zinc-400 underline-offset-4 hover:underline">
      ...
      <div className="mt-2 rounded-lg border border-black/[.06] bg-black/[.02] px-3 py-2.5 dark:border-white/[.08] dark:bg-white/[.03]">
          ...
          <ul className="mt-2 space-y-1">
            {explanation.triggers.map((t) => (
              <li key={t.rule} className="flex items-start gap-1.5 text-[11px] text-zinc-600 dark:text-zinc-300">
                ...
                <span>
                  {t.label}
                  <span className="ml-1 text-zinc-500 dark:text-zinc-400" title="the rule that surfaced this">[{t.rule}]</span>
                </span>
              </li>
            ))}
          </ul>
        ...
        {explanation.evidence.length > 0 && (
          <div className="mt-2">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Based on</p>
            <p className="mt-1 flex flex-wrap gap-1">
              {explanation.evidence.map((v, i) =>
                v.href ? (
                  ...
                  <span key={`${v.id}:${i}`} className="rounded-full bg-black/[.05] px-1.5 py-0.5 text-[10px] text-zinc-500 dark:text-zinc-400 dark:bg-white/[.06]" title={v.kind}>
                    {v.label.length > 42 ? v.label.slice(0, 41) + "…" : v.label}
                  </span>
                ),
              )}
